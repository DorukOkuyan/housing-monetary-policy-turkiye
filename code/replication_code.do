...
*===============================================================================
* 1b. ASYMMETRIC SHOCK VARIABLE
*===============================================================================

gen neg_mps = (residuals_mps < 0) * residuals_mps

label var residuals_mps "Monetary policy shock"
label var neg_mps "Negative shock interaction"

* Model:
* dy_hp(h) = beta0 * residuals_mps + beta1 * 1(residuals_mps < 0) * residuals_mps + controls
*
* beta0 = response to positive/contractionary shocks
* beta1 = difference between negative and positive shock responses
* beta0 + beta1 = response to negative/expansionary shocks
...
*===============================================================================
* A1c. AGGREGATE LP -- HP ASYMMETRIC SHOCK MODEL
*===============================================================================

matrix C_hp_asym = J(`nrows', 8, .)

forvalues h = 0/`H' {
    local nw_lag = `h' + 1
    local row = `h' + 1

    newey dy_hp`h' `shock' neg_mps `lagvars_hp' `controls', lag(`nw_lag')

    if _rc == 0 {
        matrix C_hp_asym[`row', 1] = `h'

        * beta0: positive shock response
        matrix C_hp_asym[`row', 2] = _b[`shock']
        matrix C_hp_asym[`row', 3] = _se[`shock']

        * beta1: difference between negative and positive shock responses
        matrix C_hp_asym[`row', 4] = _b[neg_mps]
        matrix C_hp_asym[`row', 5] = _se[neg_mps]

        matrix V = e(V)

        * covariance between beta0 and beta1
        matrix C_hp_asym[`row', 6] = V["`shock'", "neg_mps"]

        * p-value for asymmetry: beta1 = 0
        test neg_mps = 0
        matrix C_hp_asym[`row', 7] = r(p)

        * p-value for negative shock response: beta0 + beta1 = 0
        test `shock' + neg_mps = 0
        matrix C_hp_asym[`row', 8] = r(p)
    }
}
...
*===============================================================================
* PROGRAM: PLOT ASYMMETRIC AGGREGATE HOUSE PRICE IRF
*===============================================================================

capture program drop plot_agg_asym_difference
program define plot_agg_asym_difference
    args matname ytitle filename crit90 H graphpath
    ...
    rename `matname'1 horizon
    rename `matname'2 b_pos
    rename `matname'3 se_pos
    rename `matname'4 b_diff
    rename `matname'5 se_diff
    rename `matname'6 cov_pos_diff
    rename `matname'7 p_asym
    rename `matname'8 p_neg

    * Convert to percent
    foreach v in b_pos se_pos b_diff se_diff {
        replace `v' = `v' * 100
    }

    replace cov_pos_diff = cov_pos_diff * 10000

    * Positive shock response: beta0
    gen pos = b_pos
    gen pos_se = se_pos

    * Negative shock response slope: beta0 + beta1
    * Since the actual shock is negative, multiply by -1 for a 100bp easing response
    gen neg = - (b_pos + b_diff)

    * SE of beta0 + beta1
    gen neg_slope_se = sqrt(se_pos^2 + se_diff^2 + 2 * cov_pos_diff)

    * SE is unchanged by multiplying by -1
    gen neg_se = neg_slope_se

    gen pos_lo90 = pos - `crit90' * pos_se
    gen pos_hi90 = pos + `crit90' * pos_se
    gen pos_lo68 = pos - `crit68' * pos_se
    gen pos_hi68 = pos + `crit68' * pos_se

    gen neg_lo90 = neg - `crit90' * neg_se
    gen neg_hi90 = neg + `crit90' * neg_se
    gen neg_lo68 = neg - `crit68' * neg_se
    gen neg_hi68 = neg + `crit68' * neg_se

    twoway ///
        (rarea pos_lo90 pos_hi90 horizon, ///
            fcolor("192 57 43%10") lwidth(none)) ///
        (rarea pos_lo68 pos_hi68 horizon, ///
            fcolor("192 57 43%22") lwidth(none)) ///
        (rarea neg_lo90 neg_hi90 horizon, ///
            fcolor("44 62 80%10") lwidth(none)) ///
        (rarea neg_lo68 neg_hi68 horizon, ///
            fcolor("44 62 80%22") lwidth(none)) ///
        (line pos horizon, ///
            lcolor("192 57 43") lwidth(medthick)) ///
        (line neg horizon, ///
        ...
        , yline(0, lcolor(gs10) lpattern(solid) lwidth(thin)) ///
          ...
          xlabel(0(6)`H', labsize(vlarge)) ///
          ylabel(, labsize(vlarge) angle(horizontal) format(%9.1f) nogrid) ///
          legend(order(5 "Positive shock" 6 "Negative shock") ///
                 pos(6) ring(0) rows(1) size(large)) ///
          graphregion(color(white) margin(small)) ///
          plotregion(color(white) margin(medsmall))
...
plot_agg_asym_difference C_hp_asym "Response (%)" "irf_agg_hp_asym_difference" `crit90' `H' "`graphpath'"
